...
final class ShareExtensionTests: XCTestCase {
    ...
    func testShareIconTextMounted() {
        let share = Share(user: nil, password: nil, url: URL(string: "localhost")!, name: "test", mountPoint: "/some/path", managed: true, connected: .mounted)
        XCTAssertEqual(share.iconText, Constant.shareIconTextMounted)
    }
    func testShareIconTextUnMounted() {
        let share = Share(user: nil, password: nil, url: URL(string: "localhost")!, name: "test", mountPoint: "/some/path", managed: true, connected: .unmounted)
        XCTAssertEqual(share.iconText, Constant.shareIconTextUnMounted)
    }
    func testShareIconTextMounting() {
        let share = Share(user: nil, password: nil, url: URL(string: "localhost")!, name: "test", mountPoint: "/some/path", managed: true, connected: .mounting)
        XCTAssertEqual(share.iconText, Constant.shareIconTextMounting)
    }
    func testShareIconTextUnMounting() {
        let share = Share(user: nil, password: nil, url: URL(string: "localhost")!, name: "test", mountPoint: "/some/path", managed: true, connected: .unmounting)
